...
export default function LocationDetailCard({
  ...
  // Check whether this location has already been visited
  const isVisited = isPlaceVisited(location.id);

  const isConfirmed =
    isTheyyam &&
    (isTheyyamVerified || verificationState === 'confirmed');
  ...
  const handleVisited = () => {
    // Theyyam keeps its special verification system
    if (isTheyyam) {
      verifyTheyyamVisit();
      return;
    }

    // All other places use the normal visited system
    markPlaceVisited(location);
  };
  ...
  return (
    <div className="bg-white/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-2xl border border-black/[0.08] w-full flex flex-col max-h-full min-h-0 relative overflow-hidden text-left select-text">
      ...
      {location.image && (
        <div className="w-full h-40 sm:h-48 rounded-xl overflow-hidden mb-3 flex-shrink-0">
          <img
            src={location.image}
            alt={location.name}
